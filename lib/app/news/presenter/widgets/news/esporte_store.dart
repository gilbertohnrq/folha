...
class EsporteStore extends NotifierStore<NewsFailures, List<NewsEntity>> {
  final IFetchNewsUsecase fetchNewsUsecase;

  EsporteStore(this.fetchNewsUsecase) : super([]) {
    fetchEsportes();
  }
  ...
  Future<void> fetchEsportes() async {
    setLoading(true);
    final response = await fetchNewsUsecase.getEditoria('esportes');
     response.fold(
      (l) => setError(l),
      (r) {
        // _baseModel = r;
